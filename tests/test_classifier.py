...
class TestReplyClassifier:
    ...
    async def test_classify_interested(self, classifier):
        with patch.object(
            classifier._engine, "_call_provider", new_callable=AsyncMock, return_value="INTERESTED"
        ):
    ...
    async def test_classify_unsubscribe(self, classifier):
        with patch.object(
            classifier._engine, "_call_provider", new_callable=AsyncMock, return_value="UNSUBSCRIBE"
        ):
    ...
    async def test_classify_wrong_person(self, classifier):
        with patch.object(
            ...
            "_call_provider",
            new_callable=AsyncMock,
            return_value="WRONG_PERSON",
        ):
            result = await classifier.classify(
                "I think you've got the wrong person, I don't own this business."
            )
